...
export function ProviderCard({ usage, compact = false, detailed = false }: ProviderCardProps) {
  ...
  return (
    <section className="rounded-[8px] border border-white/10 bg-[hsl(var(--color-card)/0.38)] px-4 py-3 shadow-[0_12px_32px_rgb(0_0_0/0.18)]">
      <div className="mb-3 flex items-center justify-between gap-3">
        <h2 className="truncate text-base font-semibold text-[hsl(var(--color-text))]">{names[usage.provider]}</h2>
        <ChevronRight className="size-5 shrink-0 text-[hsl(var(--color-muted))]" aria-hidden />
      </div>

      {usage.status === "connected" ? (
        <div className="space-y-3">
          {visibleLimits.map((limit) => (
            <div key={limit.id} className="grid grid-cols-[minmax(72px,1fr)_minmax(100px,1.6fr)_auto] items-center gap-3">
              <div className="min-w-0">
                <div className="truncate text-sm text-[hsl(var(--color-text))]">{limit.label}</div>
              ...
              <div className="min-w-12 text-right text-sm font-medium text-[hsl(var(--color-text))]">
        ...
        <div className="flex items-start gap-2 text-sm text-[hsl(var(--color-muted))]">
